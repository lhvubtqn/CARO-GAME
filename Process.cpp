...
*/
bool CheckResult(int a[MaxN][MaxN], int x, int y) {
	//Hết bàn thì hòa cờ
	if (!FindNullPos(a)) {
		Result = 3;
		return true;
	...
	kq = 0;
	d = 0;
	for (i = -4; i <= 4; i++) if ((x + i >= 0) && (x + i <= gioihan))
		if (a[i + x][y] == kt)
		{
			d++;
			if (d == 5)
		...
		d = 0;
		for (i = -4; i <= 4; i++) if ((y + i >= 0) && (y + i <= gioihan))
			if (a[x][y + i] == kt)
			{
				d++;
				if (d == 5)
			...
			d = 0;
			for (i = -4; i <= 4; i++) if ((x + i >= 0) && (x + i <= gioihan) && (y + i >= 0) && (y + i <= gioihan))
				if (a[i + x][i + y] == kt)
				{
					d++;
					if (d == 5)
					{
						if ((i + x - 5 >= 0) && (i + y - 5 >= 0))
							if (a[i + x - 5][i + y - 5] == 0) kq = 1;
						if ((i + x + 1 <= gioihan) && (i + y + 1 <= gioihan))
							if (a[i + x + 1][i + y + 1] == 0) kq = 1;
					}
				}
				else d = 0;
				d = 0;
				for (i = -4; i <= 4; i++) if ((x + i >= 0) && (x + i <= gioihan) && (y - i <= gioihan) && (y - i >= 0))
					if (a[i + x][y - i] == kt)
					{
						d++;
						if (d == 5)
						{
							if ((i + x - 5 >= 0) && (y - i + 5 <= gioihan))
								if (a[i + x - 5][y - i + 5] == 0) kq = 1;
							if ((i + x + 1 <= gioihan) && (y - i - 1 >= 0))
								if (a[i + x + 1][y - i - 1] == 0) kq = 1;
						}
					}
					else d = 0;
					if (kq == 1) return true;
...
void PlayerWithCom() {
	...
	//Hiển thị các thông tin cần thiết lên màn hình
	print_info();
