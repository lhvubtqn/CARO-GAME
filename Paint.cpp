...
//Gọi các hàm tùy chỉnh console
void option_console()
{
	//điều chỉnh vị trí cửa sổ console vào giữa màn hình
	HWND consoleWindow = GetConsoleWindow();
	SetWindowPos(consoleWindow, 0, 130, 20, 0, 0, SWP_NOSIZE | SWP_NOZORDER);

	//cài đặt kích thước cửa sổ console
	resizeConsole(1050, 700);

	//tùy chỉnh title
	SetConsoleTitleW(L"GAME CARO");
}
...
void XuatKQ(int Res) {
	...
	switch (Res) {
	case 1:
		flashstring("   PLAYER1 IS THE WINNER", 15, 90);
		break;
	case 2:
		flashstring("   PLAYER2 IS THE WINNER", 15, 90);
		break;
	case 3:
		flashstring("      DRAW!", 15, 90);
		break;
	}
...
void XuatKQ_com(int Res) {
	...
	switch (Res) {
	case 1:
		flashstring("    PLAYER IS THE WINNER", 15, 90);
		break;
	case 2:
		flashstring("  COMPUTER IS THE WINNER", 15, 90);
		break;
	case 3:
		flashstring("      DRAW!", 15, 90);
		break;
...
//Hiển thị các thông tin cần thiết lên màn hình chơi game
void print_info()
{
	SetColor(15);
	Gotoxy(33, 94);
	cout << "Developed by:";
	Gotoxy(34, 94);
	cout << "1. Nguyen Xuan Vy (leader)";
	Gotoxy(35, 94);
	cout << "2. Le Hoang Vu";
	Gotoxy(36, 94);
	cout << "3. Dang Tan Tai";
	Gotoxy(37, 94);
	cout << "4. Nguyen Ha Quang";
	/* ---------------------------------- */
	SetColor(14);
	Gotoxy(39, 22);
	cout << "Arrow to move, Enter to press...";
}
#pragma endregion
